<x-dashboard-layout title="Servicios">
    <section class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="mt-2 text-headline-lg text-primary">Servicios</h1>
            <p class="mt-2 max-w-2xl text-body-md text-on-surface-variant">
                Administra los servicios publicados en el sitio y su pagina individual.
            </p>
        </div>

        <a href="{{ route('dashboard.services.create') }}" class="inline-flex h-11 items-center justify-center gap-2 rounded bg-primary px-5 text-label-md text-on-primary transition hover:opacity-90">
    ...
    <section class="overflow-hidden border border-outline-variant bg-surface-container-lowest">
        <div class="border-b border-outline-variant bg-surface-container-low px-6 py-4">
            <h2 class="text-headline-md text-primary">Listado de servicios</h2>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-outline-variant text-left text-body-sm">
                <thead class="bg-surface-container">
                    <tr>
                        <th class="px-6 py-3 font-semibold text-on-surface">Servicio</th>
                ...
                <tbody class="divide-y divide-outline-variant">
                    @forelse ($services as $service)
                        <tr>
                            <td class="px-6 py-4">
                                <div class="font-semibold text-primary">{{ $service->title }}</div>
                                @if ($service->excerpt)
                                    <p class="mt-1 max-w-md text-on-surface-variant">{{ $service->excerpt }}</p>
                                @endif
                            ...
                            <td class="px-6 py-4 text-on-surface-variant">/servicios/{{ $service->slug }}</td>
                            <td class="px-6 py-4 text-on-surface-variant">{{ $service->category?->name ?? 'Sin categoria' }}</td>
                            <td class="px-6 py-4">
                                <span class="inline-flex rounded px-3 py-1 text-label-sm {{ $service->is_published ? 'bg-secondary-container text-on-secondary-container' : 'bg-surface-container-high text-on-surface-variant' }}">
                                    {{ $service->is_published ? 'Publicado' : 'Borrador' }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('dashboard.services.edit', $service) }}" class="inline-flex h-9 items-center justify-center rounded border border-outline-variant px-3 text-label-md text-primary transition hover:bg-surface-container-low">
                                        Editar
                                    </a>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center text-on-surface-variant">
                                Todavia no hay servicios. Crea el primero para activar el listado publico.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
